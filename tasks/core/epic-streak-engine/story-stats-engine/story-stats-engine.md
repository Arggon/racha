...
### 2026-09-14 @Arggon
CORRECCIÓN del comment anterior (los backticks dispararon command substitution y se recortó texto): Implementado y mergeado en PR https://github.com/Arggon/racha/pull/11 (squash 30aa7f1). Motor puro: checks_in_month, checks_in_year, week_completion (0..100, floor, hoy incluido) con 12 tests de frontera (semana desde lunes, mes 28/30/31, 1 de enero, día sin checks). racha stats [nombre] agrega "% semana", "mes", "año" alineado al formato existente. +5 tests de integración (output nuevo con RACHA_DATA_DIR+TempDir y consumo de racha::streaks, T4). docs/FORMAT.md documentado; spec/plan streak-engine-001 en implemented (T5), spec validate ok. Gates: cargo test 35 verde (20 unit + 15 integration), clippy --all-targets 0 warnings, fmt --check limpio. Asperezas: cargo fmt reformateó storage.rs preexistente (revertido, diff enfocado; el árbol no estaba fmt-limpio antes); conflicto de rebase en tests/cli.rs con tests de list y validación de add — resuelto preservando ambos; week_completion cruza el límite de año cuando la semana calendario del 1 de enero arranca en diciembre (documentado en test).
